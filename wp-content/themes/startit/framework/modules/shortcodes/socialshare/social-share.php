...
class SocialShare implements ShortcodeInterface {
	...
	private function getSocialNetworkShareLink($net, $image) {
		...
		switch ($net) {
			case 'facebook':
				$link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(qode_startit_addslashes(get_the_title())) . '&amp;p[summary]=' . urlencode(qode_startit_addslashes(get_the_excerpt())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
				break;
			case 'twitter':
				$count_char = (isset($_SERVER['https'])) ? 23 : 22;
				$twitter_via = (qode_startit_options()->getOptionValue('twitter_via') !== '') ? ' via ' . qode_startit_options()->getOptionValue('twitter_via') . ' ' : '';
				$link = 'window.open(\'http://twitter.com/home?status=' . urlencode(qode_startit_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'google_plus':
